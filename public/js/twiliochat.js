...
var twiliochat = (function() {
  ...
  function initChannelEvents() {
    ...
    $inputText.propr('disabled', false).focus();
  }
  ...
  function leaveCurrentChannel() {
    if (tc.currentChannel) {
      return tc.currentChannel.leave().then(leftChannel => {
  ...
  function updateChannelUI(selectedChannel) {
    ...
    if (
      tc.currentChannelContainer === undefined &&
      selectedChannel.uniqueName === GENERAL_CHANNEL_UNIQUE_NAME
    ) {
      tc.currentChannelContainer = channelElement;
    }
    tc.currentChannelContainer
      .removeClass('selected-channel')
      .addClass('unselected-channel');
    channelElement
      .removeClass('unselected-channel')
